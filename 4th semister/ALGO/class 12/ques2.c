...
struct Graph* createGraph(int V, int E)
{
    ...
    graph->V = V; //assigning values to structure elements that taken form user. graph->E = E;
...
{
// This function prints the final solution printf("\nVertex\tDistance from Source Vertex\n"); int i;
...
void BellmanFord(struct Graph* graph, int source)
{
    ...
    for (i = 0; i < V; i++) StoreDistance[i] = INT_MAX;
    ...
    for (i = 1; i <= V-1; i++)
    {
        for (j = 0; j < E; j++)
        {
            ...
            int v = graph->edge[j].destination; int weight = graph->edge[j].weight;
            
            if (StoreDistance[u] + weight < StoreDistance[v]) StoreDistance[v] = StoreDistance[u] + weight;
        }
    }
...
int main()
{
    ...
    BellmanFord(graph, S);
    //passing created graph and source vertex to BellmanFord Algorithm function return 0;
